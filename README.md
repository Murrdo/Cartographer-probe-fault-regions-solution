...
Самое главное, если проблемы вдоль оси Y - в klipper до сих пор есть баг из-за которого значения из массива применяются не с ожидаемым координатам, а со смещением в 2 расстояния между координатами точек в массиве, исправляется в фаиле 
/home/orangepi/klipper/klippy/extras/axis_twist_compensation.py
Нужно найти строку - 
```
        sample_count = len(z_compensations)
```
И привести следующая за ней к такому виду:
```
...
Это исправляет смещение в 2 точки по Y НО оно будет по оси X. Сори мне лень глубже вникать в эту проблему, я писал много раз об этой проблеме на форуме клиппера - игнор. 
